...
class TradeController extends Controller
{
    ...
    public function create($offerId)
    {
        ...
        $reviews = Review::where('user_id', $offer->user_id)->with('reviewer')->take(3)->get();
        
        $rate = crypto_rate($offer);
        return view('frontend.create_trade',compact('offer','rate', 'reviews'));
    }
    ...
    public function tradeDetails($tradeCode)
    {
        ...
        return view('user.trade.details',compact('trade','messages', 'trader', 'rated', 'reviews'));
    }
    ...
    public function tradeRelease(Request $request)
    {
        ...
        $user = $trade->offer_user_id == auth()->user()->id ? $trade->trader : $trade->offerOwner;
        if($user->device_token) {
            sendFirebaseNotification('Trade Completed', "Trade has been completed. ", $user->device_token, []);
        }
    ...
    public function dispute(Request $request)
    {
        ...
        TradeChat::create([
            'user_id' => auth()->id(),
            'trade_id' => $trade->id,
            'message' => translate('Dispute created by :') . auth()->user()->name,
        ]);
