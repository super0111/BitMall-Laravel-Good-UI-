...
class UserController extends Controller{
    ...
    public function kycFormSubmit(Request $request)
    {
        ...
        foreach ($kycForm as $value) {
            ...
            if($value->type == 2 && $value->name != 'profile_photo'){
            ...
            if($value->type == 2 && $value->name == 'profile_photo'){
                $rules[$value->name] = 'image|mimes:png,jpg,jpeg|max:5120';
                if(request("$value->name")){
                    $profile_photo = MediaHelper::handleMakeImage(request("$value->name"));
                }
                unset($data[$value->name]);
                $data['image'][$value->name] = $profile_photo;
            }
        ...
        $user->kyc_info = $data;
        $user->kyc_status = 2;
        $user->photo = $profile_photo ? $profile_photo : '';
        $user->save();
