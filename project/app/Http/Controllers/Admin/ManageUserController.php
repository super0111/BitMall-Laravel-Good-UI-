...
class ManageUserController extends Controller
{
    ...
    public function details($id)
    {
        ...
        $blogs = Blog::where('type', 'user')->where('user_id', $id)->with(['category', 'user'])->get();
        $reviews = Review::where('user_id', $id)->get();
        $users = User::all();
        return view('admin.user.details',compact('user','countries', 'blogs', 'reviews', 'users'));
    }
    ...
    public function bestSeller()
    {
        ...
        $users = User::when($status,function($q) use($status) {
            ...
            return $q->where('email','like',"%$search%");
        })->where('best_seller', 1)->latest()->paginate(15);

        $allUsers = User::where('best_seller', 0)->get();

        return view('admin.user.best_seller',compact('users','search', 'allUsers'));
    }
    ...
    public function addBestSeller(Request $request)
    {
        ...
        return back()->with('success','User has been added to the list');
    }

    public function removeBestSeller(User $user)
    {        
        $user->best_seller     = 0;
        $user->save();

        return back()->with('success','User has been removed from the list');
    }
